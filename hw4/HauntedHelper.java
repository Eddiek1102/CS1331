...
public class HauntedHelper {
    ...
    /**
     * Identical to {@link #createMansionMap(Room[][], int, int)},
     * but indexes the array by X and Y coordinates instead of row and column indices.
     ...
     */
    public static String createMansionMapByXY(Room[][] rooms, int x, int y) {
        if (rooms.length == 0 || rooms[0].length == 0) {
            throw new IllegalArgumentException("You must pass in a room array with non-zero length!");
        }

        Room[][] t = new Room[rooms[0].length][rooms.length];
        for (int r = 0; r < rooms.length; ++r) {
            for (int c = 0; c < rooms[r].length; ++c) {
                t[c][r] = rooms[r][c];
            }
        }
        return createMansionMap(t, y, x);
    }
